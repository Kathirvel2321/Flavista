...
const Home = () => {
  ...
  const swiperData = [...trendingFoods, ...trendingFoods, ...trendingFoods];
  ...
  if (trendingFoods.length === 0) return <div className="min-h-screen bg-gray-900 flex items-center justify-center text-white">No trending foods found</div>;

  // Helper to get the current active food safely
  const currentFood = trendingFoods[index % trendingFoods.length] || trendingFoods[0];

  return (
    <div id="Home" className="relative min-h-screen w-full overflow-hidden bg-gray-900 flex flex-col">
      
    
      <link rel="preload" as="image" href={trendingFoods[0]?.customImage || trendingFoods[0]?.image} fetchPriority="high" />
      ...
      {trendingFoods.map((food) => (
        <div
          ...
          style={{
            backgroundImage: `url(${food.customImage || food.image})`,
      ...
      <div className="relative z-20 mt-28 mb-4 w-full flex justify-center">
        ...
        {showSuggestions && suggestions.length > 0 && (
          <div className="absolute top-full left-0 w-full mt-2 bg-[#1a1a1a]/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-fadeIn z-50">
      ...
      <div className="relative z-10 flex-grow flex flex-col items-center justify-center pb-12">
            ...
            >
              {({ isActive }) => (
                ...
                >
                  ...
                  <img
                    src={food.customImage1 || food.customImage || food.image}
                    alt={food.name}
        ...
        >
          ...
          <div className="flex items-center justify-center gap-8">
            ...
            <button 
              onClick={() => navigate(`/food/${currentFood._id}`)}
              className="px-10 py-4 bg-white text-black font-montserrat font-bold text-sm tracking-widest uppercase rounded-full shadow-xl hover:bg-yellow-400 hover:shadow-[0_0_30px_rgba(250,204,21,0.4)] transition-all duration-300 transform hover:-translate-y-1"
            >
              Order Now
            </button>
          </div>
