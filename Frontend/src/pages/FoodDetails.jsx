...
import { IoArrowForward, IoStar, IoSend, IoArrowBack, IoBagHandle, IoCheckmark } from "react-icons/io5";
...
const FoodDetails = () => {
  ...
  const food = TrendingNearYou.find((item) => item.id === Number(id));

  useEffect(() => {
    window.scrollTo(0, 0);
  }, [id]);

  if (!food) return <div className="text-white">Food not found</div>;

  const imageViews = [
    { src: food.image, className: "scale-100 hover:scale-105" }, // Full View
    {
      src: food.image,
      className: "scale-125 hover:scale-[130%]  origin-bottom-right",
    }, // Detail 1
    { src: food.image, className: "scale-150 hover:scale-[155%] origin-top" }, // Detail 2
  ];
  ...
  const [localReviews, setLocalReviews] = useState(Reviews.filter((review) => review.foodId === food.id));

  useEffect(() => {
    setLocalReviews(Reviews.filter((review) => review.foodId === food.id));
  }, [food.id]);
  ...
  const getExtras = () => {
    const cuisine = food.cuisine || "Default";
    ...
    if (
      ...
      (food.FoodName || "").toLowerCase().includes("dessert") ||
      (food.Foodname || "").toLowerCase().includes("ice cream") ||
      (food.Foodname || "").toLowerCase().includes("brownie")
    ) {
      return [
  ...
  // Logic for "You may also like" - Prioritize same cuisine, then others
  const relatedFoods = TrendingNearYou
    .filter((item) => item.id !== food.id)
    .sort((a, b) => {
      return (b.cuisine === food.cuisine ? 1 : 0) - (a.cuisine === food.cuisine ? 1 : 0);
    })
    .slice(0, 3);
      ...
      `}
        ...
        `}
          ...
          <div className="rounded-2xl md:px-6 text-white">
            <p className="text-red-600 font-mono font-bold ">{food.cuisine}</p>
              ...
              <div className="flex items-center gap-1 mt-4">
                {[...Array(5)].map((_, index) => {
                  ...
                  const fillPercentage = Math.max(
                    0,
                    Math.min(100, (ratingValue - index) * 100)
                  );
